...
public class Reporte implements ReporteLocal {
    ...
    public String generarReporteExcelFecha(String fecha1, String fecha2, String nombreArchivo, String municipalidad) throws Exception {
        ...
        try {
            ...
            try {
                ...
                archivo.close();
                auditoria.registrarAccion("Descarga de informe", nombreArchivo + ".xls");
                return url;
    ...
    public String generarReporteExcelTemporada(String año, String temporada, String nombreArchivo, String municipalidad) throws Exception {
        ...
        try {
            Collection<Registro> cr = registros.listarRegistroPorTemporada(año, temporada, municipalidad);
    ...
    public void registrarReporte(String rut) throws Exception {
        ...
        ReportesDAO rdao = dF.getReportesDAO();
        JefePlantaDAO jpdao = dF.getJefePlantaDAO();

        JefePlanta jp = jpdao.buscarPorRut(rut);

        if (jp != null) {
            entities.Reporte r = new entities.Reporte();
            Date date = new Date();
            r.setRut(jp);
            r.setFechaReporte(date);
            rdao.insert(r);
            auditoria.registrarAccion("Reporte registrado", jp.getNombreJp()+" "+jp.getApellidoJp());
        } else {
            throw new Exception("No fue posible registrar reporte");
        }
    }
}